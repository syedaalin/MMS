...
import { type EnrollmentsSettings, DEFAULT_ENROLLMENTS_SETTINGS } from "../../lib/settingsTypes";
...
/** @see {@link EnrollmentsSettings} is imported from settingsTypes.ts */
 ...
 * @returns React element.
 */
export default function EnrollmentsSettings(): React.JSX.Element {
  const [data, setData] = useState<EnrollmentsSettings>(() =>
    getObject<EnrollmentsSettings>("enrollments_settings", DEFAULT_ENROLLMENTS_SETTINGS)
  );
  const [saved, setSaved] = useState<boolean>(false);

  const upd = <K extends keyof EnrollmentsSettings>(f: K, v: EnrollmentsSettings[K]): void => {
    setData((d) => ({ ...d, [f]: v }));
    setSaved(false);
  };
