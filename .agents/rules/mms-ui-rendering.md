...
- Toast colours are **theme tokens** (`--success`/`--warning`/`--info`/`--destructive`), added in `index.css` + `tailwind.config.js`. Never hardcode `green-*`/`red-*` in toast styling. `sonner` is not used — there is one toast system (`use-toast` + `Toaster`).
